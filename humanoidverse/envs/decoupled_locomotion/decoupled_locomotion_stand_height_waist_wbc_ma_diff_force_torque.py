...
class LeggedRobotDecoupledLocomotionStanceHeightWBCForce(LeggedRobotDecoupledLocomotionStanceHeightWBC):
    def __init__(self, config, device):
        ...
        # Optional: separate curriculum/scale for external end-effector torque disturbance
        # (kept independent from force to allow ablations and safer ramp-up)
        if self.config.rewards.get("torque_scale_curriculum", False):
            self.apply_torque_scale = torch.ones(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False) * self.config.rewards.torque_scale_initial_scale
        else:
            self.apply_torque_scale = torch.ones(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _init_force_settings(self):
        ...
        # --- NEW: external end-effector torque disturbance (3D moment) ---
        # Stored similarly to forces: torque is applied in ENV_SPACE to the hand links.
        ...
        self.apply_torque_tensor = torch.zeros(self.num_envs, self.config.robot.num_bodies, 3, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        # torque application settings
        self.apply_torque_in_physics_step = self.config.get("apply_torque_in_physics_step", True)
        self.only_apply_z_torque_when_walking = self.config.get("only_apply_z_torque_when_walking", False)
        # force ranges
        ...
        # torque ranges
        self.torque_xyz_scale = torch.distributions.Dirichlet(torch.tensor([1.0, 1.0, 1.0], device=self.device)).sample((self.num_envs, ))
        self.left_torque_xyz_scale = torch.distributions.Dirichlet(torch.tensor([1.0, 1.0, 1.0], device=self.device)).sample((self.num_envs, ))
        self.right_torque_xyz_scale = torch.distributions.Dirichlet(torch.tensor([1.0, 1.0, 1.0], device=self.device)).sample((self.num_envs, ))
        ...
        self.torque_range_low = torch.zeros(self.num_envs, 3, dtype=torch.float, device=self.device, requires_grad=False)
        self.torque_range_high = torch.zeros(self.num_envs, 3, dtype=torch.float, device=self.device, requires_grad=False)
        # NOTE: apply_torque_*_range must exist in env config when enabling torque disturbance.
        self.torque_range_low[:, 0] = self.config.apply_torque_x_range[0]; self.torque_range_high[:, 0] = self.config.apply_torque_x_range[1]
        self.torque_range_low[:, 1] = self.config.apply_torque_y_range[0]; self.torque_range_high[:, 1] = self.config.apply_torque_y_range[1]
        self.torque_range_low[:, 2] = self.config.apply_torque_z_range[0]; self.torque_range_high[:, 2] = self.config.apply_torque_z_range[1]
        # force duration
        ...
        # zero torque probability (per-axis, same semantics as force)
        self.left_zero_torque_prob = self.config.get("zero_torque_prob", self.config.get("zero_force_prob", [0.2, 0.2, 0.2]))
        self.right_zero_torque_prob = self.config.get("zero_torque_prob", self.config.get("zero_force_prob", [0.2, 0.2, 0.2]))
        if isinstance(self.left_zero_torque_prob, float):
            self.left_zero_torque_prob = [self.left_zero_torque_prob] * 3
        if isinstance(self.right_zero_torque_prob, float):
            self.right_zero_torque_prob = [self.right_zero_torque_prob] * 3
        self.left_zero_torque_prob = torch.tensor(self.left_zero_torque_prob, device=self.device)
        self.right_zero_torque_prob = torch.tensor(self.right_zero_torque_prob, device=self.device)
        self.left_zero_torque = (torch.rand((self.num_envs, 3), device=self.device) < self.left_zero_torque_prob).float()
        self.right_zero_torque = (torch.rand((self.num_envs, 3), device=self.device) < self.right_zero_torque_prob).float()
        ...
        # low pass filter for applied torque (shares the same alpha)
        self.filtered_left_torque_min = torch.zeros((self.num_envs, 3), device=self.device)
        self.filtered_left_torque_max = torch.zeros((self.num_envs, 3), device=self.device)
        self.filtered_right_torque_min = torch.zeros((self.num_envs, 3), device=self.device)
        self.filtered_right_torque_max = torch.zeros((self.num_envs, 3), device=self.device)
    ...
    def _calculate_max_ee_forces(self):
        ...
        # Angular part (end-effector moment -> joint torque mapping)
        j_left_ee_joint_angular = self.j_left_ee[:, 3:6, 6:]  # Shape: (num_envs, 3, 29)
        j_right_ee_joint_angular = self.j_right_ee[:, 3:6, 6:]  # Shape: (num_envs, 3, 29)
        ...
        j_left_ee_arm_joint_angular = j_left_ee_joint_angular[:, :, self.left_arm_dof_indices]  # Shape: (num_envs, 3, 7)
        j_right_ee_arm_joint_angular = j_right_ee_joint_angular[:, :, self.right_arm_dof_indices]  # Shape: (num_envs, 3, 7)
        ...
        # --- NEW: per-axis feasible end-effector torque (moment) bounds ---
        left_ee_torque_min = torch.zeros((self.num_envs, 3), device=self.device)
        left_ee_torque_max = torch.zeros((self.num_envs, 3), device=self.device)
        right_ee_torque_min = torch.zeros((self.num_envs, 3), device=self.device)
        right_ee_torque_max = torch.zeros((self.num_envs, 3), device=self.device)

        left_ee_torque_max_delta_joint = torch.mul(
            (1.0 / (torch.abs(j_left_ee_arm_joint_angular) + 1.0e-2)),
            torch.stack([max_delta_joint_effort[:, self.left_arm_dof_indices]] * 3, dim=1),
        )
        left_ee_torque_min_delta_joint = torch.mul(
            (1.0 / (torch.abs(j_left_ee_arm_joint_angular) + 1.0e-2)),
            torch.stack([min_delta_joint_effort[:, self.left_arm_dof_indices]] * 3, dim=1),
        )
        right_ee_torque_max_delta_joint = torch.mul(
            (1.0 / (torch.abs(j_right_ee_arm_joint_angular) + 1.0e-2)),
            torch.stack([max_delta_joint_effort[:, self.right_arm_dof_indices]] * 3, dim=1),
        )
        right_ee_torque_min_delta_joint = torch.mul(
            (1.0 / (torch.abs(j_right_ee_arm_joint_angular) + 1.0e-2)),
            torch.stack([min_delta_joint_effort[:, self.right_arm_dof_indices]] * 3, dim=1),
        )
        left_ee_torque_min = torch.max(left_ee_torque_min_delta_joint, dim=2)[0]
        left_ee_torque_max = torch.min(left_ee_torque_max_delta_joint, dim=2)[0]
        right_ee_torque_min = torch.max(right_ee_torque_min_delta_joint, dim=2)[0]
        right_ee_torque_max = torch.min(right_ee_torque_max_delta_joint, dim=2)[0]

        left_ee_torque_min = torch.cat([
            left_ee_torque_min[:, 0:1] * self.torque_xyz_scale[:, 0:1],
            left_ee_torque_min[:, 1:2] * self.torque_xyz_scale[:, 1:2],
            left_ee_torque_min[:, 2:3] * self.torque_xyz_scale[:, 2:3],
        ], dim=1)
        left_ee_torque_max = torch.cat([
            left_ee_torque_max[:, 0:1] * self.torque_xyz_scale[:, 0:1],
            left_ee_torque_max[:, 1:2] * self.torque_xyz_scale[:, 1:2],
            left_ee_torque_max[:, 2:3] * self.torque_xyz_scale[:, 2:3],
        ], dim=1)
        right_ee_torque_min = torch.cat([
            right_ee_torque_min[:, 0:1] * self.torque_xyz_scale[:, 0:1],
            right_ee_torque_min[:, 1:2] * self.torque_xyz_scale[:, 1:2],
            right_ee_torque_min[:, 2:3] * self.torque_xyz_scale[:, 2:3],
        ], dim=1)
        right_ee_torque_max = torch.cat([
            right_ee_torque_max[:, 0:1] * self.torque_xyz_scale[:, 0:1],
            right_ee_torque_max[:, 1:2] * self.torque_xyz_scale[:, 1:2],
            right_ee_torque_max[:, 2:3] * self.torque_xyz_scale[:, 2:3],
        ], dim=1)
        ...
        if self.only_apply_z_torque_when_walking:
            walk_envs_idx = torch.where(self.commands[:, 4] == 1)[0]
            if len(walk_envs_idx) > 0:
                idx = walk_envs_idx
                left_ee_torque_max[idx] = torch.cat([
                    torch.zeros_like(left_ee_torque_max[idx, 0:1]),
                    torch.zeros_like(left_ee_torque_max[idx, 1:2]),
                    left_ee_torque_max[idx, 2:3],
                ], dim=1)
                left_ee_torque_min[idx] = torch.cat([
                    torch.zeros_like(left_ee_torque_min[idx, 0:1]),
                    torch.zeros_like(left_ee_torque_min[idx, 1:2]),
                    left_ee_torque_min[idx, 2:3],
                ], dim=1)
                right_ee_torque_max[idx] = torch.cat([
                    torch.zeros_like(right_ee_torque_max[idx, 0:1]),
                    torch.zeros_like(right_ee_torque_max[idx, 1:2]),
                    right_ee_torque_max[idx, 2:3],
                ], dim=1)
                right_ee_torque_min[idx] = torch.cat([
                    torch.zeros_like(right_ee_torque_min[idx, 0:1]),
                    torch.zeros_like(right_ee_torque_min[idx, 1:2]),
                    right_ee_torque_min[idx, 2:3],
                ], dim=1)
        ...
        if self.use_lpf:
            ...
            self.filtered_left_torque_min = self.force_filter_alpha * left_ee_torque_min + (1 - self.force_filter_alpha) * self.filtered_left_torque_min
            self.filtered_left_torque_max = self.force_filter_alpha * left_ee_torque_max + (1 - self.force_filter_alpha) * self.filtered_left_torque_max
            self.filtered_right_torque_min = self.force_filter_alpha * right_ee_torque_min + (1 - self.force_filter_alpha) * self.filtered_right_torque_min
            self.filtered_right_torque_max = self.force_filter_alpha * right_ee_torque_max + (1 - self.force_filter_alpha) * self.filtered_right_torque_max
            left_ee_torque_min = self.filtered_left_torque_min
            left_ee_torque_max = self.filtered_left_torque_max
            right_ee_torque_min = self.filtered_right_torque_min
            right_ee_torque_max = self.filtered_right_torque_max
        return (
            left_ee_force_min,
            left_ee_force_max,
            right_ee_force_min,
    ...
    def _scale_torques(
        ...
        """Scale and sample end-effector torque (moment) disturbance.

        Uses the same phase variables as force so that wrench trajectories are synchronized.
        """
        left_ee_torque_phased = left_ee_torque_min + (left_ee_torque_max - left_ee_torque_min) * self.left_ee_apply_force_phase
        right_ee_torque_phased = right_ee_torque_min + (right_ee_torque_max - right_ee_torque_min) * self.right_ee_apply_force_phase

        left_hand_torque = left_ee_torque_phased * self.apply_torque_scale + torch.rand((self.num_envs, 3), device=self.device)
        right_hand_torque = right_ee_torque_phased * self.apply_torque_scale + torch.rand((self.num_envs, 3), device=self.device)

        left_hand_torque *= (1 - self.left_zero_torque)
        right_hand_torque *= (1 - self.right_zero_torque)

        left_hand_torque = torch.clip(left_hand_torque, self.torque_range_low, self.torque_range_high)
        right_hand_torque = torch.clip(right_hand_torque, self.torque_range_low, self.torque_range_high)
        return left_hand_torque, right_hand_torque
    ...
    def _calculate_ee_forces(self):
        if self.max_force_estimation:
            ...
            left_ee_torque_min = right_ee_torque_min = self.torque_range_low
            left_ee_torque_max = right_ee_torque_max = self.torque_range_high
        ...
        left_hand_torque, right_hand_torque = self._scale_torques(
            left_ee_torque_min,
            left_ee_torque_max,
            right_ee_torque_min,
            right_ee_torque_max,
        ...
        self.left_ee_apply_torque = quat_rotate_inverse(self.base_quat, left_hand_torque.clone())
        self.right_ee_apply_torque = quat_rotate_inverse(self.base_quat, right_hand_torque.clone())
        ...
        self.apply_torque_tensor[:, self.left_hand_link_index, :] = left_hand_torque
        self.apply_torque_tensor[:, self.right_hand_link_index, :] = right_hand_torque
    ...
    def _apply_force_in_physics_step(self):
        ...
        # # ===== DEBUG torque tensor =====
        # if not hasattr(self, "_dbg_torque_step"):
        #     self._dbg_torque_step = 0
        # self._dbg_torque_step += 1

        # N = 200  # 每 N 步打印一次
        # if self._dbg_torque_step % N == 0:
        #     li = self.left_hand_link_index
        #     ri = self.right_hand_link_index

        #     left_tau = self.apply_torque_tensor[:, li, :]   # (num_envs, 3)
        #     right_tau = self.apply_torque_tensor[:, ri, :]

        #     left_norm = left_tau.norm(dim=-1)
        #     right_norm = right_tau.norm(dim=-1)

        #     print(
        #         f"[TorqueDebug step={self._dbg_torque_step}] "
        #         f"L_tau(mean={left_norm.mean().item():.3f}, max={left_norm.max().item():.3f}) "
        #         f"R_tau(mean={right_norm.mean().item():.3f}, max={right_norm.max().item():.3f}) "
        #         f"L_vec0={left_tau[0].tolist()} R_vec0={right_tau[0].tolist()}"
        #     )
        # # ===============================

        if self.config.get("apply_torque_in_physics_step", False):
    ...
    def _resample_force_settings(self, env_ids):
        ...
        if env_ids.numel() > 0:  # Only update if env_ids is not empty
            ...
            self.torque_xyz_scale[env_ids] = torch.distributions.Dirichlet(
                torch.tensor([1.0, 1.0, 1.0], device=self.device)
            ).sample((len(env_ids), ))
        ...
        self.left_zero_torque[env_ids] = (torch.rand((len(env_ids), 3), device=self.device) < self.left_zero_torque_prob).float()
        self.right_zero_torque[env_ids] = (torch.rand((len(env_ids), 3), device=self.device) < self.right_zero_torque_prob).float()
    ...
    def resample_motion(self):
        ...
        self.filtered_left_torque_max *= 0.0
        self.filtered_left_torque_min *= 0.0
        self.filtered_right_torque_max *= 0.0
        self.filtered_right_torque_min *= 0.0
    ...
    def _resample_motion_times(self, env_ids):
        ...
        self.filtered_left_torque_max[env_ids] *= 0.0
        self.filtered_left_torque_min[env_ids] *= 0.0
        self.filtered_right_torque_max[env_ids] *= 0.0
        self.filtered_right_torque_min[env_ids] *= 0.0
    ...
    def set_is_evaluating(self, command=None):
        ...
        # Apply torque scale (defaults to `force_level` if not explicitly set)
        torque_level = self.config.get("eval_torque_level", force_level)
        if torque_level == 0:
            self.apply_torque_scale = torch.zeros(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False)
        elif torque_level == 1:
            self.apply_torque_scale = torch.ones(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False) / 2
        elif torque_level == 2:
            self.apply_torque_scale = torch.ones(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def reset_envs_idx(self, env_ids, target_states=None, target_buf=None):
        ...
        if self.config.rewards.get("torque_scale_curriculum", False):
            self._update_torque_scale_curriculum(env_ids)
        if self.config.rewards.get("command_height_scale_curriculum", False):
    ...
    def _update_torque_scale_curriculum(self, env_ids):
        """Implement torque curriculum (mirrors force curriculum).

        This is optional and only active when `rewards.torque_scale_curriculum` is True.
        """
        env_ids_scale_up_mask = self.episode_length_buf[env_ids] > self.config.rewards.torque_scale_up_threshold
        env_ids_scale_up = env_ids[torch.where(env_ids_scale_up_mask)[0]]
        env_ids_scale_down_mask = self.episode_length_buf[env_ids] < self.config.rewards.torque_scale_down_threshold
        env_ids_scale_down = env_ids[torch.where(env_ids_scale_down_mask)[0]]
        self.apply_torque_scale[env_ids_scale_up] += self.config.rewards.torque_scale_up
        self.apply_torque_scale[env_ids_scale_down] -= self.config.rewards.torque_scale_down
        self.apply_torque_scale[env_ids] = torch.clip(
            self.apply_torque_scale[env_ids],
            self.config.rewards.torque_scale_min,
            self.config.rewards.torque_scale_max,
        )
    ...
    def _get_obs_left_ee_apply_torque(self):
        # return the torque (moment) exerted on the left ee (hand)
        return self.left_ee_apply_torque

    def _get_obs_right_ee_apply_torque(self):
        # return the torque (moment) exerted on the right ee (hand)
